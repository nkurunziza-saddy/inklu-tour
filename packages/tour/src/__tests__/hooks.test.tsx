  ...
  return {
    TourEngine: class MockTourEngine {
      ...
      setProps(open: boolean, stepIndex: number, steps: any) {
        if (this.state.open === open && this.state.stepIndex === stepIndex) return;
        this.state = { ...this.state, open, stepIndex, currentStep: steps[stepIndex] ?? null, mounted: open };
...
describe("Tour React Integration", () => {
  ...
  it("updates state when calling hooks", () => {
    const tours = [{
      id: "tour-1",
      steps: [{ id: "step-1", target: "#foo" }, { id: "step-2", target: "#bar" }]
    }];
