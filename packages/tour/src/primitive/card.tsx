...
const CardContext = React.createContext<{ arrowStyle: React.CSSProperties } | null>(null);
...
export const Card = React.forwardRef<HTMLDivElement, TourCardProps>(
	...
	const { rects, currentStep, isWaiting } = useTourContext();
					...
					...(pos
						? {
								...
								transition: isTransitioning
									? `top ${TOUR_ANIMATION_DURATION}ms cubic-bezier(0.22, 1, 0.36, 1), left ${TOUR_ANIMATION_DURATION}ms cubic-bezier(0.22, 1, 0.36, 1)`
									: "none",
							}
						: {
								bottom: "24px",
...
export interface TourButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
	asChild?: boolean;
}
...
export const PreviousButton = React.forwardRef<HTMLButtonElement, TourButtonProps>(
	({ onClick, asChild, ...props }, ref) => {
